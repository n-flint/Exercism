...
class ClockTest < Minitest::Test
  def test_on_the_hour
    # skip
    assert_equal "08:00", Clock.new(hour: 8).get_time
  end

  def test_past_the_hour
    # skip
    assert_equal "11:09", Clock.new(hour: 11, minute: 9).get_time
  end

  def test_midnight_is_zero_hours
    # skip
    assert_equal "00:00", Clock.new(hour: 24).get_time
  end

  def test_hour_rolls_over
    # skip
    assert_equal "01:00", Clock.new(hour: 25).get_time
  end

  def test_hour_rolls_over_continuously
    # skip
    assert_equal "04:00", Clock.new(hour: 100).get_time
  end

  def test_sixty_minutes_is_next_hour
    # skip
    assert_equal "02:00", Clock.new(hour: 1, minute: 60).get_time
  end

  def test_minutes_roll_over
    # skip
    assert_equal "02:40", Clock.new(minute: 160).get_time
  end

  def test_minutes_roll_over_continuously
    # skip
    assert_equal "04:43", Clock.new(minute: 1723).get_time
  end

  def test_hour_and_minutes_roll_over
    # skip
    assert_equal "03:40", Clock.new(hour: 25, minute: 160).get_time
  end

  def test_hour_and_minutes_roll_over_continuously
    # skip
    assert_equal "11:01", Clock.new(hour: 201, minute: 3001).get_time
  end

  def test_hour_and_minutes_roll_over_to_exactly_midnight
    # skip
    assert_equal "00:00", Clock.new(hour: 72, minute: 8640).get_time
  end

  def test_negative_hour
    # skip
    assert_equal "23:15", Clock.new(hour: -1, minute: 15).get_time
  end

  def test_negative_hour_rolls_over
    # skip
    assert_equal "23:00", Clock.new(hour: -25).get_time
  end

  def test_negative_hour_rolls_over_continuously
    # skip
    assert_equal "05:00", Clock.new(hour: -91).get_time
  end

  def test_negative_minutes
    # skip
    assert_equal "00:20", Clock.new(hour: 1, minute: -40).get_time
  end

  def test_negative_minutes_roll_over
    # skip
    assert_equal "22:20", Clock.new(hour: 1, minute: -160).get_time
  end

  def test_negative_minutes_roll_over_continuously
    # skip
    assert_equal "16:40", Clock.new(hour: 1, minute: -4820).get_time
  end

  def test_negative_sixty_minutes_is_previous_hour
    # skip
    assert_equal "01:00", Clock.new(hour: 2, minute: -60).get_time
  end

  def test_negative_hour_and_minutes_both_roll_over
    # skip
    assert_equal "20:20", Clock.new(hour: -25, minute: -160).get_time
  end

  def test_negative_hour_and_minutes_both_roll_over_continuously
    # skip
    assert_equal "22:10", Clock.new(hour: -121, minute: -5810).get_time
  end

  # def test_add_minutes
  #   skip
  #   clock1 = Clock.new(hour: 10, minute: 0)
  #   assert_equal "10:03", (clock1.get_time + Clock.new(minute: 3)).to_s
  # end
  ...
  def test_clocks_with_same_time
    ...
    assert clock1.get_time == clock2.get_time
  end
  ...
  def test_clocks_a_minute_apart
    ...
    clock2 = Clock.new(hour: 15, minute: 37)
    refute clock1.get_time == clock2.get_time
  end
